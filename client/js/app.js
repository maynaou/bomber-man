...
export function App(gameState, players = [], seconds = {}) {
      ...
      h(
        ...
        playerPositions.map((player) => {
          ...
          return h(
            "div",
            {
              class: `player-stat ${isCurrentUser ? "current-player" : ""} ${
                !isAlive ? "dead-player" : ""
              }`,
            },
            [
  ...
  function handle_win(playerPositions) {
    ...
    return h("div", { id: "congratulations" }, [
      ...
      h(
        ...
        `${playerPositions[0].username} Win The Game`
      ),
      h(
        "button",
        {
  ...
  function handle_Tie() {
    ...
    return h("div", { id: "congratulations" }, [
      ...
      h(
        "p",
        { id: "congratulations-message" },
        `No victory this time. It's a tie!`
      ),
      h(
        "button",
        {
  ...
  function createMapFromDataWithAbsolutePositioning(
    mapData,
    rows,
    cols,
    activeBombs = [],
    ...
    playerPositions.forEach((player) => {
      ...
      const playerElement = h("div", {
        class: `player-absolute ${player.direction} player-${player.count} ${
          isDamaged ? " damaged" : ""
        }`,
        style: `
            position: absolute;
      ...
      "div",
      {
        ...
        onmousedown: (e) => {
          const currentPlayer = document.getElementById(
            `player-controlled-${globalUsername}`
          );
  ...
  if (gameState === "game_start") {
    ...
    if (playerPositions.length === 1) {
      return h("div", { class: "game-container" }, [
        ...
        mapData
          ? createMapFromDataWithAbsolutePositioning(
              mapData,
              rows,
              cols,
              activeBombs,
    ...
    return h(
      "div",
      {
        ...
        onkeydown: (e) => {
          const currentPlayer = playerPositions.find(
            (p) => p.username === globalUsername
          );
          if (currentPlayer) {
            handlemoveplayer(
              e,
              globalUsername,
              currentPlayer.pixelX,
              currentPlayer.pixelY
            );
          }
        },
        tabindex: 0,
  ...
  return h("div", { class: "container" }, [
    ...
    h("div", { class: "game-box" }, [h("p", {}, "État de jeu inconnu...")]),
  ]);
}
