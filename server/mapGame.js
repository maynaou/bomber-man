...
export class GenerateMapGame {
  constructor(rows, cols, room) {
    this.rows = rows;
    this.cols = cols;
    this.room = room; // Store room reference instead of importing globally
  ...
  generatePlayerPositions() {
    ...
    return Array.from(this.room.players.values()).map((player, index) => {
      return {
    ...
    for (let r = 0; r < this.rows; r++) {
      ...
      for (let c = 0; c < this.cols; c++) {
        ...
        if (r === 0 || r === this.rows - 1 || c === 0 || c === this.cols - 1) {
          ...
          this.playerPositions.some(
            (p) => Math.abs(p.r - r) <= 1 && Math.abs(p.c - c) <= 1
          )
        ) {
          cellType = "empty";
        } else {
  ...
  movePlayerByPixels(currentPixelX, currentPixelY, direction, playerId) {
    ...
    if (direction === "Space" || direction === " ") {
      ...
      if (hasBomb) {
        const playerBombs = this.activeBombs.filter(
          (bomb) => bomb.playerId === playerId
        );
  ...
  placeBombs(playerId) {
    const player = this.playerPositions.find((p) => p.id === playerId);
    const playerActiveBombs = this.activeBombs.filter(
      (bomb) => bomb.playerId === playerId
    );
    const hasReachedMaxBombs =
      playerActiveBombs.length >= player.stats.maxBombs;
  ...
  checkGameEnd() {
    ...
    const alivePlayers = this.playerPositions.filter(
  ...
  explodeBomb(r, c, playerId) {
    ...
    for (const { dr, dc } of directions) {
      for (let step = 1; step <= flameRange; step++) {
        const nr = r + dr * step;
        const nc = c + dc * step;
        const hasBonus = this.bonuses
          ? this.bonuses.find((b) => b.r === nr && b.c === nc)
          : null;
        ...
        if (nr >= 0 && nr < this.rows && nc >= 0 && nc < this.cols) {
          ...
          if (target === "block") {
            ...
            ["speed", "flame", "powerUp"].includes(target) &&
            !hasBonus.shouldRestore
          ) {
            this.mapData[nr][nc] = "cutted";
            if (hasBonus) {
        ...
        if (player) {
          ...
          switch (bonusType) {
            case "speed":
              ...
              break;
            case "flame":
              player.stats.flameRange = Math.min(
                player.stats.flameRange + 1,
                3
              ); // Max 8
              break;
            case "powerUp":
